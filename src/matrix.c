...
/**
 * @brief Compute determinant of a square matrix (recursive helper).
 * @param a Input matrix.
 * @param n Size of the square matrix.
 * @return Determinant of the matrix.
 */
static int mathi_determinant_recursive(int **a, int n)
{
    if(n == 1) return a[0][0];
    ...
    for(int c = 0; c < n; c++)
    {
        ...
        int sign = (c % 2 == 0) ? 1 : -1;
        det += sign * a[0][c] * mathi_determinant_recursive(sub, n-1);
...
{
    return mathi_determinant_recursive(a, n);
}
