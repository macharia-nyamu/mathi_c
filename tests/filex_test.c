...
void test_file_write_and_exists() {
    ...
    int res = file_write(path, "Hello Mathi!", strlen("Hello Mathi!"));
...
void test_file_size_and_read() {
    ...
    int read_res = file_read(path, buffer, sizeof(buffer));
...
void test_file_append() {
    ...
    const char *path = "test_file.txt";
    int res = file_append(path, " Appended");
    printf("file_append('%s') result = %d\n", path, res);
    assert(res);

    char buffer[1024] = {0};
    file_read(path, buffer, sizeof(buffer));
...
void test_file_copy() {
    ...
    int res = file_copy(src, dest);
    printf("file_copy('%s','%s') result = %d\n", src, dest, res);
    assert(res);
    int exists = file_exists(dest);
    printf("file_exists('%s') = %d\n", dest, exists);
    assert(exists);
}
...
void test_file_delete() {
    ...
    int res1 = file_delete(path1);
    printf("file_delete('%s') result = %d\n", path1, res1);
    assert(res1);

    int res2 = file_delete(path2);
    printf("file_delete('%s') result = %d\n", path2, res2);
    assert(res2);

    int exists1 = file_exists(path1);
    int exists2 = file_exists(path2);
