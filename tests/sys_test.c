...
void test_sys_env() {
    ...
    set_env(var_name, "", 1); // clear previous value
    ...
    int res = set_env(var_name, "123", 1); // overwrite
    assert(res == 1);
    ...
    res = set_env(var_name, "456", 0); // do not overwrite
    ...
    res = set_env(var_name, "456", 1); // overwrite
    assert(res == 1);
